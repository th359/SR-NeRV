...
from sr.model import ResidualSRModel
...
class HNeRV(nn.Module):
    def __init__(self, args):
        ...
        self.super_resolution = ResidualSRModel(scale_factor=args.scale)
    ...
    def forward(self, input, input_embed=None, encode_only=False):
        ...
        lr_out = OutImg(self.head_layer(output), self.out_bias)
        img_out = self.super_resolution(lr_out)

        if torch.cuda.is_available():
            torch.cuda.synchronize()
        dec_time = time.time() - dec_start

        return  img_out, lr_out, embed_list, dec_time
...
class HNeRVDecoder(nn.Module):
    def __init__(self, model):
        ...
        self.super_resolution = model.super_resolution
    ...
    def forward(self, img_embed):
        ...
        output = OutImg(output, self.out_bias)
        output = self.super_resolution(output)
